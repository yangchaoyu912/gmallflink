...
public class HotUrlApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //3. 从网络端口中获取数据
        SingleOutputStreamOperator<ApacheLog> apacheLogDS = env.socketTextStream("hadoop102", 9999).map(line -> {
            ...
            //因为数据源的事件时间可能是无序的，需要使用watermark对数据进行时间标记 延时 2 秒
            @Override
    ...
    private static class TopNProcessFunc extends KeyedProcessFunction<Long,UrlViewCount,String> {
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            ArrayList<UrlViewCount> itemCounts = Lists.newArrayList(iterator);

            //按照count排序
            itemCounts.sort(new Comparator<UrlViewCount>() {
                @Override
                //按照降序
            ...
            for (int i = 0; i < Math.min(topSize, itemCounts.size()); i++) {
                //取出数据
                UrlViewCount itemCount = itemCounts.get(i);
